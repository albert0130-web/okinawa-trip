// Version: 5.1.0 - Restore Missing Accommodation Button
// @ts-nocheck
...
import { 
  ...
  Utensils, ShoppingBag, Hotel, CloudRain
} from 'lucide-react';
...
const INITIAL_ITINERARY = [
  {
    ...
    location: '台北 ➔ 沖繩',
    events: [
      { id: 'd1-1', time: '14:10', title: '接小孩放學', location: '學校', type: 'transport', image: '/images/day1_1.png', desc: '要準備好護照。' },
      { id: 'd1-2', time: '16:00', title: '抵達桃園 T1', location: '桃園機場', travelTime: '50分', image: '/images/day1_1.png', desc: '要準備好護照。' },
      { id: 'd1-3', time: '18:20', title: '樂桃 MM930 起飛', location: '桃園機場', travelTime: '等待', image: '/images/day1_2.jpg', desc: '飛往那霸機場-飛行時間90分鐘' },
      { id: 'd1-4', time: '20:50', title: '抵達那霸機場', location: '那霸機場', travelTime: '飛行 90分', image: '/images/day1_4.png' },
      { id: 'd1-5', time: '21:10', title: '出境', location: '那霸機場', image: '/images/day1_5.png', desc: '準備好入境資料QR Code' },
      { id: 'd1-6', time: '21:30', title: '搭乘單軌電車', location: '那覇空港駅', travelTime: '電車 10分', image: '/images/day1_6.png', desc: `坐往壺川站\n票價:\n大人: 290\n小孩: 150` },
      { id: 'd1-7', time: '21:55', title: '入住飯店', location: 'Glory island okinawa -SOBE-', travelTime: '步行 15分', image: '/images/day1_7.png', desc: 'Hotels.com 行程編號：73305401005050', introImages: ['/images/day1_7.png'] },
    ]
  },
  {
    date: '12/5',
  ...
  },
  {
    ...
    events: [
      ...
      { id: 'd3-8', time: '21:00', title: '回飯店', location: 'Family Condo Chatan Hills', travelTime: '車程 15分', image: '/images/day3_6.png' },
    ]
  },
  {
    date: '12/7',
  ...
  },
  {
    ...
    events: [
      ...
      { id: 'd5-5', time: '19:00', title: '晚餐', location: 'Private Condo Chatan Jagaru', travelTime: '車程 20分', image: '' },
    ]
  },
  {
    date: '12/9',
...
const WeatherIcon = ({ code }) => {
  ...
  if (code >= 51 && code <= 67) return <CloudRain className="w-10 h-10 text-blue-500" />;
...
export default function OkinawaTravelApp() {
  ...
  // New Inputs for Food/Shop/Accom with Image
  ...
  const handleAddNewEvent = async () => {
    ...
    const newData = itineraryData.map(day => {
      if (day.date === currentDayDate) {
        const updatedEvents = [...day.events, newEventObj].sort((a, b) => a.time.localeCompare(b.time));
    ...
    if (isLocalMode) {
      setCustomEvents([...customEvents, {id: Date.now(), ...newEventObj}]);
    } else {
      try {
        await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'itinerary_events'), newEventObj);
      } catch(e) {
        console.error(e);
        setIsLocalMode(true);
        setCustomEvents([...customEvents, {id: Date.now(), ...newEventObj}]);
      ...
      const newData = itineraryData.map(day => ({
        ...
        events: day.events.map(ev => {
          if (ev.id === showEventDetail.id) {
             ...
             setShowEventDetail(updated); // Update current view
             return updated;
          }
          return ev;
        })
  ...
  const handleDeleteInfo = (category, id) => {
    setInfoData({
      ...infoData,
      [category]: infoData[category].filter(item => item.id !== id)
    });
  };
  ...
  const handleAddInfo = (category) => {
    ...
    setInfoData({
      ...infoData,
      [category]: [...infoData[category], newItem]
    });
    setNewInfoInput({ ...newInfoInput, [category]: {} });
  };

  // --- Food, Shop, Accommodation ---
  const handleAddFood = () => {
  ...
  const handleAddAccom = () => {
    ...
    setInfoData({
       ...infoData,
       accommodation: [...infoData.accommodation, { id: Date.now(), ...newAccom }]
    });
  ...
  const handleDeleteAccom = (id) => {
    setInfoData({
      ...infoData,
      accommodation: infoData.accommodation.filter(i => i.id !== id)
    });
  };
        ...
        fetch(`https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lon}&current=temperature_2m,relative_humidity_2m,weather_code,is_day&hourly=temperature_2m,weather_code,precipitation_probability&timezone=auto&forecast_days=2`)
          ...
          .then(data => {
            ...
            if(data.hourly) {
               ...
               const next24 = [];
               for(let i=0; i<24; i++) {
                  const idx = i + currentHour; // Simple offset
  ...
  const handleAddBudget = async () => {
    ...
    try { 
       await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'budget'), data); 
       setNewItemName(''); setNewItemCost(''); setBudgetImage(null); 
    } catch(e) { 
       setIsLocalMode(true); 
       setBudgets([ {id: Date.now().toString(), ...data}, ...budgets]);
  ...
  return (
    <div 
      className="min-h-screen font-sans pb-40 bg-cover bg-center bg-no-repeat bg-fixed transition-all duration-500"
      style={{ 
    ...
    >
      {/* Header (Customizable) */}
      <div className="p-5 pt-[max(2rem,env(safe-area-inset-top))] bg-white/80 backdrop-blur-md border-b border-white/20 rounded-b-[2.5rem] shadow-sm z-10 relative transition-all">
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-2xl font-black text-blue-900 tracking-tight mb-1 drop-shadow-sm">OKINAWA TRIP 🏖️</h1>
            <p className="text-sm text-slate-600 font-medium tracking-wide">2025.12.04 - 12.09</p>
          </div>
          <label className="bg-white/50 p-2 rounded-full cursor-pointer hover:bg-white transition shadow-sm border border-white/50">
      ...
      <div className="mt-4 px-4 overflow-x-auto snap-x flex space-x-3 no-scrollbar pb-2">
        {itineraryData.map((item, index) => (
          <button key={index} onClick={() => setActiveTab(index)} className={`snap-center flex-shrink-0 w-20 rounded-2xl flex flex-col items-center justify-center transition-all duration-300 border ${activeTab === index ? 'bg-blue-600 text-white shadow-lg scale-105' : 'bg-white/80 text-gray-500'}`}>
            <span className="text-xs font-medium opacity-80">{item.date}</span><span className="text-lg font-bold">{item.day}</span>
          </button>
        ))}
      </div>

      {/* Dashboard */}
      <div className="px-4 mt-4 space-y-3 relative z-20">
        <div className="grid grid-cols-2 gap-3">
          <div onClick={() => setShowTimeModal(true)} className="bg-white/90 backdrop-blur-sm p-4 rounded-[2rem] shadow-sm h-32 flex flex-col justify-center cursor-pointer active:scale-95 transition-transform">
            <div className="text-xs text-blue-500 font-bold mb-1 flex items-center gap-1"><Clock className="w-3 h-3"/> TIME <ChevronDown className="w-3 h-3"/></div>
          ...
          <div onClick={() => setShowWeatherModal(true)} className="bg-white/90 backdrop-blur-sm p-4 rounded-[2rem] shadow-sm h-32 flex flex-col justify-between cursor-pointer active:scale-95 transition-transform">
             <div className="absolute top-3 right-3"><LocateFixed className="w-4 h-4 text-emerald-500 animate-pulse" /></div>
             <div className="flex justify-between items-start pt-2">
                <div><div className="text-xs text-gray-400 font-bold uppercase tracking-wider">{localWeather.name}</div><div className="text-2xl font-bold">{localWeather.temp}°</div></div>
                <WeatherIcon code={localWeather.code} />
             </div>
             <div className="flex items-center text-orange-400 text-xs font-medium"><Droplets className="w-3 h-3 mr-1" /> {localWeather.hum}%</div>
          </div>
        </div>
      </div>
      ...
      <div className="px-2 mt-8 pb-10 overflow-hidden">
        <h2 className="text-sm font-bold text-white/90 mb-6 pl-4 tracking-widest uppercase drop-shadow-md">Itinerary</h2>
          ...
          {displayEvents.map((event, idx) => {
            ...
            return (
              <div key={event.id} className={`relative flex items-center ${isLeft ? 'justify-start pl-2' : 'justify-end pr-2'}`}>
                ...
                <div className={`absolute top-1/2 -translate-y-1/2 ${isLeft ? 'right-2 text-right' : 'left-2 text-left'} w-12`}><span className="font-bold text-slate-800 bg-white/80 px-1 rounded font-mono text-sm block">{event.time}</span></div>
                
                <div className="relative w-[85%] h-56 rounded-[2rem] shadow-xl bg-white group z-10 flex flex-col overflow-hidden transition-all hover:shadow-2xl">
                    <button 
                      onClick={(e) => { e.stopPropagation(); handleDeleteEvent(itineraryData[activeTab].date, event.id); }}
                      className="absolute top-2 right-2 z-30 bg-white/80 p-1.5 rounded-full text-red-500 shadow-sm hover:bg-red-500 hover:text-white transition"
                    >
      ...
      <div className="fixed bottom-0 left-0 right-0 p-3 pb-[env(safe-area-inset-bottom)] bg-gradient-to-t from-white/90 via-white/90 to-transparent z-50">
        <div className="bg-slate-800 text-white rounded-[2rem] p-2 shadow-2xl flex justify-between items-center h-20 max-w-lg mx-auto border border-slate-700">
          ...
          <button onClick={() => setShowAccommodation(true)} className="flex-1 flex flex-col items-center justify-center active:scale-90 transition">
            <Hotel className="w-6 h-6 mb-1 text-purple-300" /><span className="text-[10px] font-bold">住宿</span>
          </button>
      ...
      {/* Shopping Modal (Updated with Image) */}
      {showShopping && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-end justify-center animate-fade-in">
           <div className="bg-white w-full sm:w-96 h-[80vh] rounded-t-[2rem] flex flex-col shadow-2xl">
                 ...
                 {shoppingList.map(s => (
                   <div key={s.id} className="bg-white p-3 rounded-xl shadow-sm border border-pink-100 flex justify-between items-center gap-3">
                      ...
                      <div className="flex-1">
                        ...
                        {s.shop && <span className="text-xs bg-pink-100 text-pink-600 px-2 py-0.5 rounded-full">{s.shop}</span>}
                      </div>
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-end justify-center animate-fade-in">
          <div className="bg-white w-full sm:w-96 h-[80vh] rounded-t-[2rem] flex flex-col shadow-2xl">
            ...
            <div className="p-4 bg-purple-50/50 space-y-3">
               ...
               <button onClick={handleAddAccom} className="w-full bg-purple-600 text-white py-2 rounded-lg font-bold shadow">新增住宿</button>
            </div>
            <div className="p-4 space-y-4 overflow-y-auto flex-1">
              {infoData.accommodation.map((a,i)=>(
                <div key={a.id || i} className="bg-white p-5 rounded-2xl shadow-sm border border-purple-100 relative">
                  <button onClick={()=>handleDeleteAccom(a.id)} className="absolute top-3 right-3 text-gray-300 hover:text-red-500"><Trash2 className="w-4 h-4"/></button>
      ...
      {showEventDetail && (
        <div className="fixed inset-0 z-[60] bg-white flex flex-col animate-fade-in">
           ...
           <div className="h-[45vh] relative flex-shrink-0 bg-slate-100">
              <img src={showEventDetail.image} className="w-full h-full object-cover" onClick={() => setFullscreenImage(showEventDetail.image)}/>
              ...
              <div className="mt-8 mb-8">
                  ...
                  <div className="flex space-x-3 overflow-x-auto pb-2 no-scrollbar">
                    {showEventDetail.introImages && showEventDetail.introImages.map((img, idx) => (
                      <img key={idx} src={img} className="h-32 w-48 object-cover rounded-xl shadow-md flex-shrink-0 cursor-pointer" onClick={() => setFullscreenImage(img)}/>
                    ))}
                  </div>
              </div>
                 ...
                 <div className="grid grid-cols-2 gap-4">
                    {moments.filter(m=>m.eventTitle===showEventDetail.title).map(m=>(
                      <div key={m.id} className="relative">
                         <img src={m.image} className="aspect-square rounded-2xl object-cover shadow-md bg-gray-50 cursor-pointer w-full" onClick={() => setFullscreenImage(m.image)}/>
      ...
      {/* ... AddEvent, Budget, Time Modals ... (Keep existing) */}
      {/* (Skipped for brevity but must be kept in final code) */}
      {showAddEvent && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-end justify-center animate-fade-in">
          <div className="bg-white w-full sm:w-96 rounded-t-[2rem] h-[85vh] flex flex-col shadow-2xl">
            <div className="p-6 border-b flex justify-between items-center">
              <h3 className="font-bold text-xl">新增行程</h3>
              <button onClick={() => setShowAddEvent(false)} className="bg-gray-100 p-2 rounded-full"><X className="w-5 h-5" /></button>
            </div>
            <div className="p-6 flex-1 space-y-4 overflow-y-auto">
               ...
               <button onClick={handleAddNewEvent} className="w-full bg-blue-600 text-white py-4 rounded-2xl font-bold shadow-lg">確認新增</button>
            </div>
          </div>
        </div>
      )}
